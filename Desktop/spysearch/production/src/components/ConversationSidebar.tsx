...
export const ConversationSidebar = forwardRef<ConversationSidebarRef, ConversationSidebarProps>(({ 
  ...
  const loadConversations = async () => {
    try {
      const response = await fetch(API_CONFIG.ENDPOINTS.GET_TITLES);
      if (response.ok) {
        ...
        const conversationPromises = titles.map(async (title: string) => {
          try {
            const messageResponse = await fetch(API_CONFIG.ENDPOINTS.LOAD_MESSAGE, {
              method: 'POST',
  ...
  const deleteConversation = async (title: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      const response = await fetch(API_CONFIG.ENDPOINTS.DELETE_MESSAGE, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title }),
      });
...
export const conversationManager = {
  saveMessage: async (title: string, role: 'user' | 'assistant', content: string) => {
    try {
      const response = await fetch(API_CONFIG.ENDPOINTS.APPEND_MESSAGE, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: {
